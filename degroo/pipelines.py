...
class DegrooPipeline:

    # Database setup
    conn = sqlite3.connect('degroo.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `degroo`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM degroo WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO degroo (date, title, link, content)"
